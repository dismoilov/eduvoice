...
Ожидается: `All checks passed!` от линтера и `103 passed` от тестов.
...
открыт лишь порт панели 9094, и то под паролем. Проверка снаружи:
...
```json
{"status": "ok", "active_calls": 0, "provider": "fake"}
```
...
Что это доказывает: вопрос распознан, ответ взят из `content/faq.yaml` по ключевым словам
(без обращения к модели, поэтому 302 мс), ответ проигран звонящему.
