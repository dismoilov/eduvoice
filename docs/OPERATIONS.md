...
| `LOUDNESS_MARGIN` | 10 | во сколько раз кадр должен быть громче комнаты, чтобы считаться речью |
| `LOUDNESS_FLOOR_MIN` | 30 | самое тихое, чем считается комната. Вместе с порогом даёт 300 — значение, которое держится и в зале, и на тихой линии |
| `BARGE_IN_MARGIN` | 10 | то же для перебивания ответа |
...
Если в зале помощник «не слышит» — поднимать `LOUDNESS_FLOOR_MIN`; если слышит лишнее и
переспрашивает — тоже его. Порог = `LOUDNESS_MARGIN × LOUDNESS_FLOOR_MIN`. Разбирать по
записи: включить `EDUVOICE_DUMP_AUDIO=1`, позвонить, затем прогнать файл детектором.
