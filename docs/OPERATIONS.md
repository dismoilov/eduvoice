...
| `LAW_TIMEOUT_S` | 12 | отдельный бюджет модели на ответ по выдержкам из закона: три выдержки читаются дольше, чем одна строка классифицируется, а это последний рубеж перед «не понял» |
